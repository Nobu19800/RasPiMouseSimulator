...
void RasPiMouseObj::setCurrentVelocity(double px, double py, double pa)
{
	current_px = px;
	current_py = py;
	current_pa = pa;
...
void RasPiMouseObj::setUltrasonicSensorPos(double x, double y, double z)
{
	current_ultrasonicSensor_x = x;
	current_ultrasonicSensor_y = y;
	current_ultrasonicSensor_z = z;
}
void RasPiMouseObj::setColourSensorPos(double x, double y, double z)
{
...
bool RasPiMouseObj::getRightTouch(double limit)
{
...
bool RasPiMouseObj::getLeftTouch(double limit)
{
...
double RasPiMouseObj::calcUltrasonicSensorDistance(double x, double y, double z)
{
...
double RasPiMouseObj::calcColourSensorDistance(double x, double y, double z)
{
...
void RasPiMouseObj::resetUltrasonicSensorDistance()
{
	current_ultrasonicSensorData = 100;
}
void RasPiMouseObj::resetColourSensorDistance()
{
...
{
	RasPiMouseBlock.m = DEFAULT_BODY_MASS;
	RasPiMouseBlock.lx = DEFAULT_BODY_LENGTH;
	RasPiMouseBlock.ly = DEFAULT_BODY_WIDTH;
	RasPiMouseBlock.lz = DEFAULT_BODY_HEIGHT;
	RasPiMouseBlock.x = DEFAULT_BODY_X;
	RasPiMouseBlock.y = DEFAULT_BODY_Y;
	RasPiMouseBlock.z = DEFAULT_BODY_Z + offset_z;
	RasPiMouseBlock.red = 0.9;
	RasPiMouseBlock.green = 0.9;
	RasPiMouseBlock.blue = 0.9;
...
void RasPiMouseSimulatorObj::makeRobot()
{
	...
	//RasPiMouseBlock.joint = dJointCreateFixed(world, 0);
	//dJointAttach(RasPiMouseBlock.joint, RasPiMouseBlock.body, 0);
	//dJointSetFixed(RasPiMouseBlock.joint);

	setHinge(&wheelLeft, &RasPiMouseBlock);
	setHinge(&wheelRight, &RasPiMouseBlock);
	setBall(&wheelBall, &RasPiMouseBlock);
	setFixed(&touchSensorLeft[0], &RasPiMouseBlock);
	setFixed(&touchSensorLeft[1], &touchSensorLeft[0]);
	setFixed(&touchSensorLeft[2], &touchSensorLeft[1]);
	setFixed(&touchSensorRight[0], &RasPiMouseBlock);
	setFixed(&touchSensorRight[1], &touchSensorRight[0]);
	setFixed(&touchSensorRight[2], &RasPiMouseBlock);
	...
	setFixed(&gyroSensor[0], &RasPiMouseBlock);
	setFixed(&gyroSensor[1], &gyroSensor[0]);
	setFixed(&gyroSensor[2], &gyroSensor[1]);
	setFixed(&colourSensor, &RasPiMouseBlock);
	...
	setFixed(&mmotor, &RasPiMouseBlock);
  ...
  //if ((o1 == plane.geom) || (o2 == plane.geom)) return;
   ...
   static const int N = 20;
	...
	if ((o1 == ultrasonicSensor_ray && o2 == ground) || (o2 == ultrasonicSensor_ray && o1 == ground)){
		...
		RasPiMouse.calcUltrasonicSensorDistance(contact[0].geom.pos[0], contact[0].geom.pos[1], contact[0].geom.pos[2]);
		return;
	}
	...
	if ((o1 == colourSensor_ray && o2 == ground) || (o2 == colourSensor_ray && o1 == ground)){
		...
		RasPiMouse.calcColourSensorDistance(contact[0].geom.pos[0], contact[0].geom.pos[1], contact[0].geom.pos[2]);
		return;
	}
...
void RasPiMouseSimulatorObj::control()
{
	...
	//RasPiMouse.velocity_mmotor = 0.1;
	controlHinge(&ultrasonicSensor[0], RasPiMouse.target_mangle, RasPiMouse.velocity_mmotor);

	RasPiMouse.left_touch_value = dJointGetSliderPosition(touchSensorLeft[3].joint);
	RasPiMouse.right_touch_value = dJointGetSliderPosition(touchSensorRight[3].joint);
	...
	double current_vx = v * cos(RasPiMouse.current_pa);
	double current_vy = v * sin(RasPiMouse.current_pa);
	double current_va = o;
...
*@brief 超音波距離センサを設定
*/
void RasPiMouseSimulatorObj::setUltrasonicSensorRay()
{
	dVector3 pos0, pos1;
	...
	RasPiMouse.setUltrasonicSensorPos(pos0[0], pos0[1], pos0[2]);
	...
	RasPiMouse.resetUltrasonicSensorDistance();
}
/**
*@brief カラーセンサを設定
*/
void RasPiMouseSimulatorObj::setColourSensorRay()
{
	dVector3 pos0, pos1;
...
void RasPiMouseSimulatorObj::destroyRobot()
{
	...
	dBodyDestroy(RasPiMouseBlock.body);
	...
	dGeomDestroy(RasPiMouseBlock.geom);
